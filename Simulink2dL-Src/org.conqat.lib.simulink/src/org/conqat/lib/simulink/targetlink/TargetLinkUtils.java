...
import static org.conqat.lib.simulink.model.SimulinkConstants.PARAM_BACKGROUND_COLOR;
...
public class TargetLinkUtils {
	...
	public static void filterSyntheticBlocks(SimulinkModel model) throws SimulinkModelBuildingException {
		...
		CCSMAssert.isNotNull(subsystem,
				String.format("The SimFrame Block has no sub-block with name \"%s\"", NAME_SUBSYSTEM));

		SimulinkBlock block = subsystem.getSubBlock(name);
		CCSMAssert.isNotNull(block, String.format("The SimFrame/Subsystem block has no sub-block with name %s", name));

		simFrameBlock.replace(block, PARAM_POSITION, PARAM_BACKGROUND_COLOR);
	 ...
	 */
	private static class SyntheticBlockFindingVisitor implements IVisitor<SimulinkBlock, NeverThrownRuntimeException> {
		...
		public void visit(SimulinkBlock element) throws NeverThrownRuntimeException {
			...
			if ("TL_SimFrame".equals(maskType)) {
				CCSMAssert.isTrue(simFrameBlock == null, "We assume that there's only one "
