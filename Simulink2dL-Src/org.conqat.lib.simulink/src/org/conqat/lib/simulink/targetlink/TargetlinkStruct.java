...
/* package */class TargetlinkStruct {
	...
	/** Recursively add parameters to the map. */
	private void addParameters(Map<String, String> map, String prefix) {

		for (String paramName : parameters.keySet()) {
			map.put(prefix + TargetLinkDataResolver.PARAMETER_SEPARATOR + paramName, parameters.get(paramName));
		}

		for (String childName : children.keySet()) {
			TargetlinkStruct child = children.get(childName);
			child.addParameters(map, prefix + TargetLinkDataResolver.PARAMETER_SEPARATOR + childName);
		}
	}
	...
	public String toString() {
		...
		for (String childName : CollectionUtils.sort(children.keySet())) {
			...
			result.append("' -> ");
			result.append(StringUtils.prefixLines(children.get(childName).toString(), TWO_SPACES, false));
			result.append(LINE_SEPARATOR);
		}
