...
public class SimulinkLineConstraint implements ILocalConstraint<SimulinkLine> {
	...
	/** See class comment. */
	@Override
	public void checkLocalConstraint(SimulinkLine element) throws ConstraintViolationException {
		if (element.getSrcPort().getBlock().getParent() != element.getDstPort().getBlock().getParent()) {
			throw new ConstraintViolationException("Line " + element + " crosses subsystem boundaries.", element);
		}
	}
