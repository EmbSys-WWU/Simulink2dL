...
public class SimulinkBlockTypeConstraint implements ILocalConstraint<SimulinkBlock> {
	...
	/** See class comment. */
	@Override
	public void checkLocalConstraint(SimulinkBlock element) throws ConstraintViolationException {
		if (StringUtils.isEmpty(element.getType())) {
			throw new ConstraintViolationException("Block " + element + " has no type.", element);
		}
	}
}