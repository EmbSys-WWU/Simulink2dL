...
/* package */class TransitionCloneUtils {
	...
	private static void duplicateTransition(StateflowTransition transition, IStateflowNodeContainer<?> orig,
			IStateflowNodeContainer<?> clone, Map<String, StateflowNodeBase> idToNode) {
	 ...
	 */
	private static void extractInTransitionsRecursive(IStateflowNodeContainer<?> container,
		...
		for (StateflowNodeBase node : container.getNodes()) {
			...
			if (node instanceof IStateflowNodeContainer<?>) {
				extractInTransitionsRecursive((IStateflowNodeContainer<?>) node, transitions);
			}
		}

		if (container instanceof StateflowState && ((StateflowState) container).isSubChart()) {
			extractInTransitionsRecursive(((StateflowState) container).getSubViewer(), transitions);
		}
	}
	...
	private static void fillIdToNodeMap(IStateflowNodeContainer<?> container, Map<String, StateflowNodeBase> idToNode) {
		...
		if (container instanceof StateflowState && ((StateflowState) container).isSubChart()) {
			fillIdToNodeMap(((StateflowState) container).getSubViewer(), idToNode);
		}
	}
	 ...
	 * exists.
	 */
	private static IStateflowElement<?> getLowestCommonAncestor(IStateflowElement<?> elem1,
			IStateflowElement<?> elem2) {
