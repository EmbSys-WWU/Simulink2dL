...
public class SimulinkModelWalker implements IMeshWalker<Object, NeverThrownRuntimeException> {
	...
	public Collection<Object> getAdjacentElements(Object element) {
		...
		if (element instanceof StateflowState) {
			StateflowState state = (StateflowState) element;
			if (state.getSubViewer() != null) {
				elements.add(state.getSubViewer());
			}
		}
		...
		CCSMAssert.isFalse(elements.contains(null), "Element " + element + " has a null adjancency.");
