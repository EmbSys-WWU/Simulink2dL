...
public class ExternalRatingPartitionTable {
	...
	private static final Pattern EXTERNAL_TABLE_PATTERN = Pattern.compile(".*" + EXTERNAL_TABLE_TAG + "[ _]+(\\S+) *",
	...
	private void createResolver(String className, String[] args) throws ExternalRatingTableException {
		try {
	...
	private void handleResolverCreationException(Exception e) throws ExternalRatingTableException {
		resolver = null;
		throw new ExternalRatingTableException("Could not instantiate resolver: " + e.getMessage());
	}

	/** Determines the class used for the resolver. */
	@SuppressWarnings("unchecked")
	private static Class<? extends IFilePathResolver> determineResolverClass(String className)
			throws ExternalRatingTableException {
		Class<? extends IFilePathResolver> resolverClass;
		try {
			resolverClass = (Class<? extends IFilePathResolver>) Class.forName(className, true,
					Thread.currentThread().getContextClassLoader());
		} catch (ClassNotFoundException e) {
			throw new ExternalRatingTableException("Could not find resolver class " + className);
		}
		if (!IFilePathResolver.class.isAssignableFrom(resolverClass)) {
			throw new ExternalRatingTableException("Given resolver class " + className + " does not implement "
	...
	private String determineRelativePath() {
		...
		return resolver.getRelativeFilePath(FileSystemUtils.normalizeSeparators(accessor.getFilePath()));
	}
	...
	public void updateTable(List<String> partitionTags) {
		...
		String content = StringUtils.concat(partitionTags, StringUtils.LINE_SEPARATOR);
