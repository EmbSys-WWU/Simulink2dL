...
public class ConstraintValidator {
	...
	public void checkConstaints(Object o) throws ConstraintViolationException {
		checkConstaints(o, RethrowingExceptionHandler.<ConstraintViolationException> getInstance());
	}
	...
	public <X extends Exception> void checkConstaints(Object o,
			IExceptionHandler<ConstraintViolationException, X> handler) throws X {
	 ...
	 */
	public <T, X_WALKER extends Exception> void validateTree(T root, ITreeWalker<T, X_WALKER> walker)
			throws ConstraintViolationException, X_WALKER {

		validateTree(root, walker, RethrowingExceptionHandler.<ConstraintViolationException> getInstance());
	}
	...
	public <T, X extends Exception, X_WALKER extends Exception> void validateTree(T root,
			ITreeWalker<T, X_WALKER> walker, IExceptionHandler<ConstraintViolationException, X> handler)
			throws X, X_WALKER {

		VisitorUtils.visitAllPreOrder(root, walker, new CheckVisitor<T, X>(handler));
	}
	 ...
	 *             if the walker throws an exception.
	 */
	public <T, X_WALKER extends Exception> void validateMesh(T start, IMeshWalker<T, X_WALKER> walker)
			throws ConstraintViolationException, X_WALKER {

		validateMesh(start, walker, RethrowingExceptionHandler.<ConstraintViolationException> getInstance());
	}
	...
	public <T, X extends Exception, X_WALKER extends Exception> void validateMesh(T start,
			IMeshWalker<T, X_WALKER> walker, IExceptionHandler<ConstraintViolationException, X> handler)
			throws X, X_WALKER {

		VisitorUtils.visitAllDepthFirst(start, walker, new CheckVisitor<T, X>(handler));
	}
	 ...
	 * method.
	 */
	private class CheckVisitor<T, X extends Exception> implements IVisitor<T, X> {
