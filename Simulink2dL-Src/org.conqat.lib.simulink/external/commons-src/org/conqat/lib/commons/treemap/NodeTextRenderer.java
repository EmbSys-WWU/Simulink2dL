...
public class NodeTextRenderer implements ITreeMapRenderer {
	...
	private static boolean enoughSpace(Rectangle2D nodeArea) {
		return nodeArea.getWidth() > 3 * TEXT_PADDING && nodeArea.getHeight() > 3 * TEXT_PADDING;
	}
	...
	private void drawText(String text, Rectangle2D availableSpace, Graphics2D graphics) {
		...
		// cut text to size
		String fittedText = clipTextToWidth(text, availableSpace.getWidth(), graphics);
		if (fittedText.length() < text.length()) {
			fittedText += "...";
		}

		// compute text position
		int x = (int) availableSpace.getCenterX() - (actualWidth(fittedText, graphics) / 2);
		int y = (int) availableSpace.getCenterY() + (actualHeight(fittedText, graphics) / 2);
	...
	private String clipTextToWidth(String text, double width, Graphics2D graphics) {
		...
		if (separationPattern != null && actualWidth(text, graphics) > availableWidth) {
		...
		while (text.length() > 0 && actualWidth(text, graphics) > availableWidth) {
