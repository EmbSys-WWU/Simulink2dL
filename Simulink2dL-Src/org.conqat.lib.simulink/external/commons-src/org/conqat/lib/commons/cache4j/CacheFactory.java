...
public class CacheFactory {
	...
	private CacheCreationRule defaultRule = new CacheCreationRule("", ECacheThreadSupport.NONE, ECachingStrategy.OFF,
			0);
	...
	private static CacheCreationRule parseRule(String line) throws CacheRuleParsingException {
	...
	public static <K, V, X extends Exception> ICache<K, V, X> obtainCache(String name,
			IParameterizedFactory<V, K, X> factory) {
		return getInstance().obtainCacheByRules(name, factory);
	}

	/** Returns a cache based on the provided name and the configured rules. */
	private synchronized <K, V, X extends Exception> ICache<K, V, X> obtainCacheByRules(String name,
			IParameterizedFactory<V, K, X> factory) {
	...
	public static <K, V, X extends Exception> ICache<K, V, X> obtainCache(Class<?> clazz,
			IParameterizedFactory<V, K, X> factory) {
	...
	public static final class CacheCreationRule {
		...
		/** Constructor. */
		public CacheCreationRule(Pattern pattern, ECacheThreadSupport threadSupport, ECachingStrategy cachingStrategy,
				int cachingStrategyParameter) {
		...
		/** Constructor. */
		public CacheCreationRule(String pattern, ECacheThreadSupport threadSupport, ECachingStrategy cachingStrategy,
				int cachingStrategyParameter) throws PatternSyntaxException {
		...
		public <K, V, X extends Exception> ICache<K, V, X> createCache(String name,
			...
			return threadSupport.createCache(name, factory,
					(ICacheBackend) cachingStrategy.getBackend(cachingStrategyParameter));
		}
	}
	...
	private static final class CacheInfo {
		...
		public void appendStatistics(StringBuilder sb) {
			...
			sb.append(cache.getName() + ";" + creationRule.threadSupport.name() + ";"
					+ creationRule.cachingStrategy.name() + ";" + creationRule.cachingStrategyParameter + ";"
					+ cache.getHits() + ";" + cache.getMisses() + ";" + cache.getMissCostMillis() + StringUtils.LINE_SEPARATOR);
		}
	}
}