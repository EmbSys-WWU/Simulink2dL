...
public class GenericTypeResolver {
	...
	/**
	 * Initializes the generic parameter lookup table by comparing for each super
	 * class and interface the type parameters with the actual type arguments. This
	 * process then is repeated recursively. It is important to fill from the
	 * current class before going to the super class, as the super class may
	 * reference parameters used in this class.
	 */
	...
	/**
	 * Fill the generic parameter lookup map from an explicit (class, type) pair by
	 * comparing the type parameters with the actual type parameters (if the class
	 * is generic at all).
	 * 
	 * @param clazz
	...
	private void fillInParameters(Class<?> clazz, Type type) {
		if (type instanceof ParameterizedType) {
			...
			for (int i = 0; i < typeParameters.length; ++i) {
				parameterLookup.put(typeParameters[i], resolveGenericType(actualTypeArguments[i]));
			}
		}
	}

	/**
	 * Returns the actual type from a (potentially) generic type. If the argument is
	 * a plain class, it is returned, otherwise a lookup in the internal generic
	 * parameter map is performed. For parameterized types (e.g.
	 * <code>List&lt;String&gt;</code> the raw type (here: List) is returned.
	 * <p>
	 * Note that this only works for return values and parameters of methods
	 * belonging to the class for which this instance was constructed for. Otherwise
	 * the behavior is undefined (either returning nonsense or throwing an
	 * exception).
	 * 
	 * @param genericType
	 *            a type such as returned from {@link Method#getGenericReturnType()}
	 *            or {@link Method#getGenericParameterTypes()}.
	 */
	public Class<?> resolveGenericType(Type genericType) {
		...
		if (genericType instanceof TypeVariable<?>) {
			check(parameterLookup.containsKey(genericType), "All generic parameters should be bound.");
		...
		if (genericType instanceof GenericArrayType) {
			...
			return Array.newInstance(resolveGenericType(gat.getGenericComponentType()), 0).getClass();
		}

		check(false, "Generic types should be either concrete classes, type variables, or parametrized types: "
				+ genericType.getClass());
		return null; // this line is never reached
	}

	/**
	 * This is an assertion method to simplify changing the type of error handling.
	 * Many of the assertions rather document our assumptions about the JVM, than
	 * really checking errors.
	 */
