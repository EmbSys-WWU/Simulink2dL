...
public class SerializedEntityParser {
	...
	private SerializedEntityParser(DataInputStream din) {
		CCSMAssert.isTrue(din.markSupported(), "Our parser requires an input stream that supports marking! "
	...
	private int parseObject() throws IOException {
		return new SerializedObject(din, pool, this, parseClassDesc()).getHandle();
	}
	...
	private int parseArray() throws IOException {
		return new SerializedArrayObject(din, pool, this, parseClassDesc()).getHandle();
	}
	...
	private int parseEnum() throws IOException {
		return new SerializedEnumLiteral(pool, this, parseClassDesc()).getHandle();
	}
