...
public final class ClassType {
	...
	private void mergeInClass(Class<?> c) throws TypesNotMergableException {
		if (c.isInterface()) {
			interfaces.add(c);
		} else if (baseClass.equals(c) || ReflectionUtils.isAssignable(baseClass, c)) {
			...
			throw new TypesNotMergableException("Types " + c + " and " + baseClass + " could not be merged!");
		}
	}
	...
	public ClassType intersect(ClassType classType) {
		ClassType result = new ClassType(ReflectionUtils.determineCommonBase(getBaseClass(), classType.getBaseClass()));
