...
public class Rating {
	...
	/* package */static final String COLORS_REGEX = StringUtils.concat(ETrafficLightColor.values(), "|");

	/** Pattern used to extract the rating. */
	private static final Pattern RATING_PATTERN = Pattern.compile(
			"(" + RATING_TAG + "|" + OLD_RATING_TAG + ") +(" + COLORS_REGEX + ") +Hash:? *([a-fA-F0-9]*)",
	...
	private boolean checkRating(String line) {
		...
		oldStyleRating = ratingMatcher.group(1).equalsIgnoreCase(OLD_RATING_TAG);
		storedRating = EnumUtils.valueOf(ETrafficLightColor.class, ratingMatcher.group(2));
	 ...
	 * invariant about changes in whitespace or the use of different
	 * encodings.</li>
	 ...
	 * 
	 */
	private static void updateHash(String line, MessageDigest md5) {
	...
	public static String updateRating(String content, ETrafficLightColor newColor) {
		...
		String replacement = constructRatingTag(newColor, new Rating(content, newColor).getExpectedHashString());
