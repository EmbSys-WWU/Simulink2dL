...
public class HTMLWriter extends XMLWriter<EHTMLElement, EHTMLAttribute> {
	...
	public void addStdHeader(String encoding) {
		...
		addPublicDocTypeDefintion(EHTMLElement.HTML, "-//W3C//DTD XHTML 1.0 Frameset//EN",
	...
	public void addAttribute(EHTMLAttribute attribute, Object value) {
		if (!getCurrentElement().allowsAttribute(attribute)) {
			throw new HTMLWriterException("Attribute " + attribute + " not allowed for element " + getCurrentElement());
		}
	 ...
	 */
	private static void assertCssDeclarationBlock(Object value) {
		if (!(value instanceof CSSDeclarationBlock)) {
			throw new HTMLWriterException("The argument for STYLE and CLASS attributes must be a "
