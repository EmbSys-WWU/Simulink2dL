...
public abstract class StringUndoStackBase {
	...
	public void undo() {
		CCSMAssert.isTrue(canUndo(), "Must be allowed to undo!");
		currentVersion = join(deltas.get(currentVersionIndex--).backwardPatch(split(currentVersion)));
	...
	public void redo() {
		CCSMAssert.isTrue(canRedo(), "Must be allowed to redo!");
		currentVersion = join(deltas.get(++currentVersionIndex).forwardPatch(split(currentVersion)));
		setModelFromString(currentVersion);
		fireStackChanged();
	}
