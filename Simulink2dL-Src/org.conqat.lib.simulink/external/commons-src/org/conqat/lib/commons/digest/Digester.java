...
public class Digester {
	...
	/**
	 * MD5 Digesters used organized by thread. This is used to avoid recreation of
	 * digesters, while keeping the code thread safe (i.e. each thread has its own
	 * instance).
	 */
	...
	/**
	 * SHA-1 Digesters used organized by thread. This is used to avoid recreation of
	 * digesters, while keeping the code thread safe (i.e. each thread has its own
	 * instance).
	 */
	...
	/**
	 * Computes an MD5 hash for a string. The hash is always 32 characters long and
	 * only uses characters from [0-9A-F].
	 */
	...
	/**
	 * Computes an MD5 hash for a byte array. The hash is always 32 characters long
	 ...
	 * before MD5 computation, so that the resulting MD5 hash is independent of the
	 * order of the strings in the collection.
	 */
	public static String createMD5Digest(Collection<String> bases) {
		List<String> sortedBases = CollectionUtils.sort(bases);
		return createMD5Digest(StringUtils.concat(sortedBases, StringUtils.EMPTY_STRING));
	}

	/**
	 * Computes an SHA-1 hash for a byte array and returns the binary hash (i.e. no
	 * string conversion).
	 */
	...
	/**
	 * Computes an MD5 hash for a byte array and returns the binary hash (i.e. no
	 * string conversion).
	 */
