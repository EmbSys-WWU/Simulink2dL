...
public class CSSDeclarationBlock {
		...
		for (int i = 0; i < values.length; i += 2) {
			if (!(values[i] instanceof ECSSProperty)) {
				throw new IllegalArgumentException(
						"Expected CSS property as parameter " + i + " instead of " + values[i].getClass());
			}
			if (!(values[i + 1] instanceof String)) {
				throw new IllegalArgumentException("Expected property value (String) as parameter " + (i + 1)
						+ " instead of " + values[i + 1].getClass());
			}
	...
	public CSSDeclarationBlock setMargin(String value) {
		return setProperties(value, MARGIN_BOTTOM, MARGIN_LEFT, MARGIN_RIGHT, MARGIN_TOP);
	}
	...
	public CSSDeclarationBlock setPadding(String value) {
		return setProperties(value, PADDING_BOTTOM, PADDING_LEFT, PADDING_RIGHT, PADDING_TOP);
	}
	...
	public CSSDeclarationBlock setBorderWidth(String width) {
		return setProperties(width, BORDER_BOTTOM_WIDTH, BORDER_LEFT_WIDTH, BORDER_RIGHT_WIDTH, BORDER_TOP_WIDTH);
	}
	...
	public CSSDeclarationBlock setBorderStyle(String style) {
		return setProperties(style, BORDER_BOTTOM_STYLE, BORDER_LEFT_STYLE, BORDER_RIGHT_STYLE, BORDER_TOP_STYLE);
	}
	...
	public CSSDeclarationBlock setBorderColor(String color) {
		return setProperties(color, BORDER_BOTTOM_COLOR, BORDER_LEFT_COLOR, BORDER_RIGHT_COLOR, BORDER_TOP_COLOR);
	}
	...
	public void writeOut(PrintStream ps, String indent) {
		Map<ECSSProperty, String> result = new EnumMap<ECSSProperty, String>(ECSSProperty.class);
		fillFullPropertyMap(result);

		for (ECSSProperty property : result.keySet()) {
			ps.println(indent + property.getName() + ": " + result.get(property) + ";");
		}
	}
