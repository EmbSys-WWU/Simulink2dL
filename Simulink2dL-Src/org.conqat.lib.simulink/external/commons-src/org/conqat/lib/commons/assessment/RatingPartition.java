...
public class RatingPartition {
	...
	private static final Pattern PARTITION_PATTERN = Pattern.compile(
			".*" + PARTITION_TAG + " +(" + Rating.COLORS_REGEX + ") +Hash: *([a-fA-F0-9]*)( .*)?",
	...
	/** Constructor. */
	/* package */ RatingPartition(ETrafficLightColor rating, String hash, String name) {
	...
	/* package */void setLinesAndContent(int startLine, int endLine, List<String> content) {
	...
	/* package */String getTag() {
		...
		return PARTITION_TAG + " " + rating.name() + " Hash: " + hash + " " + getName();
	}
}