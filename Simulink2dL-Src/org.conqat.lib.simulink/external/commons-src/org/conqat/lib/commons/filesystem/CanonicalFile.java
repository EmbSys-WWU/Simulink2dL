...
public class CanonicalFile extends File {
	...
	public CanonicalFile[] listFiles() {
		...
		return canonize(files);
	}

	/**
	 * Same as {@link File#listFiles(FileFilter)} but returns canonical files. If
	 * for some strange reason the files below a canonical file cannot be canonized,
	 * this may throw an {@link AssertionError}.
	 *
	 * If this abstract pathname does not denote a directory, then this method
	 * returns {@code null}.
	 */
	...
	/**
	 * Same as {@link File#listFiles(FilenameFilter)} but returns canonical files.
	...
	/**
	 * Canonizes list of filenames. If a file could not be canonized, this throws an
	 * {@link AssertionError}.
	 */
