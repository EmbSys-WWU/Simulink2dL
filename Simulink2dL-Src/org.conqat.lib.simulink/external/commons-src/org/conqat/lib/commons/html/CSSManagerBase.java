...
public abstract class CSSManagerBase {
	...
	public void addDefaultDeclaration(EHTMLElement element, ECSSPseudoClass pseudoClass, CSSDeclarationBlock block) {
		if (defaultDeclarations.containsKey(element, pseudoClass)) {
			throw new IllegalStateException("May not add element " + element + " twice.");
		}
	...
	/**
	 * Returns the name of the CSS class used for this block. If the block is not
	 * yet known, it is registered with this manager.
	 */
	 ...
	 * subclasses. However it must be made sure, that the class names returned are
	 * unique and do not overlap with HTML element names.
	 */
	...
	/**
	 * Write all selectors with their blocks to the given stream. The format is the
	 * one usually used in CSS files. This merely calls
	...
	protected void writeOutDefaultDeclarations(PrintStream ps) {
		for (EHTMLElement element : defaultDeclarations.getFirstKeys()) {
			for (ECSSPseudoClass pseudocssClass : defaultDeclarations.getSecondKeys(element)) {
				String selector = element.getName() + pseudocssClass.getName();
				writeBlock(defaultDeclarations.getValue(element, pseudocssClass), selector, ps);
			}
		}
	}
	...
	protected void writeOutDeclarations(PrintStream ps) {
		for (Map.Entry<CSSDeclarationBlock, String> entry : classNames.entrySet()) {
			writeBlock(entry.getKey(), "." + entry.getValue(), ps);
		}
	}

	/** Writes a single block/selector pair to the given stream. */
	private static void writeBlock(CSSDeclarationBlock block, String selector, PrintStream ps) {
		ps.print(selector);
		ps.println(" {");
		block.writeOut(ps, "  ");
		ps.println("}");
