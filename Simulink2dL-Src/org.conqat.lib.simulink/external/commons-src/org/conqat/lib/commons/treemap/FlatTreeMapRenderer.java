...
public class FlatTreeMapRenderer implements ITreeMapRenderer {
	 ...
	 * arranged in a striped pattern, which is arranged horizontally or vertically
	 * depending on the aspect ratio of the rectangle.
	 */
	private void fillRectMultiColor(Graphics2D graphics, Rectangle2D rect, MultiColor multiColor) {
		...
		for (int i = 0; i < multiColor.size(); ++i) {
			...
			if (width > height) {
				subrect = new Rectangle2D.Double(x + offset * width, y, width * relativeFrequency, height);
			} else {
				subrect = new Rectangle2D.Double(x, y + offset * height, width, height * relativeFrequency);
			}
