...
public class SortedCounterSet<E extends Comparable<? super E>> extends CounterSet<E> {
	...
	/**
	 * Obtain the sum of all values in a certain range of elements. This operation
	 * runs in time <code>O(n*log(n))</code> where <code>n</code> is the size of
	 * this set, as the list of items is sorted each time.
	 * 
	 * @param firstElement
	 *            the first element to include. If the element is not present in the
	 *            list, the smallest element greater than this one is used.
	 * @param lastElement
	 *            the last element to include. If the element is not present in the
	...
	public int getRangeSum(E firstElement, E lastElement) {
		...
		int firstIndex;
		// this uses natural ordering if compartor is null
		firstIndex = Collections.binarySearch(elementList, firstElement, comparator);
		...
		int lastIndex;
		lastIndex = Collections.binarySearch(elementList, lastElement, comparator);
