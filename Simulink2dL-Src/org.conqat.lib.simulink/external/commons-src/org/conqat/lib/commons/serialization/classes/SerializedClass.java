...
public class SerializedClass extends SerializedClassBase {
	...
	/** Direct constructor. */
	public SerializedClass(String name, int serialVersionUid, byte classDescriptionFlags, SerializedEntityPool pool) {
		super(pool);
		this.name = name;
	...
	public void addField(SerializedFieldBase newField) {
		CCSMAssert.isTrue(getField(newField.getName()) == null, "Field with this name already exists!");
		fields.add(newField);
	}

	/**
	 * Removes the field of given name from this class. Note that the instances
	 * of this class have to be adjusted accordingly, otherwise you will get
	...
	public static SerializedClass createSimpleEnum(SerializedEntityPool pool, String enumName) {
		SerializedClass resultClass = new SerializedClass(enumName, 0, ENUM_CLASS_DESCRIPTION_FLAGS, pool);

		SerializedClass enumClass = pool.findClass(Enum.class.getName());
		if (enumClass == null) {
			enumClass = new SerializedClass(Enum.class.getName(), 0, ENUM_CLASS_DESCRIPTION_FLAGS, pool);
		}
