...
public abstract class SerializedClassBase extends SerializedEntityBase {
	...
	/**
	 * Class annotations. These are either byte arrays for block data, or integer
	 * values representing handles.
	 */
	private final List<Object> classAnnotations = new ArrayList<>();

	/** Constructor for parsing. */
	protected SerializedClassBase(DataInputStream din, SerializedEntityPool pool, SerializedEntityParser parser)
			throws IOException {
		super(pool);
	...
	/**
	 * Parses the class content. This is called from the constructor, so fields of
	 * sub classes might not be initialized.
	 */
	protected abstract void parseClass(DataInputStream din, SerializedEntityPool pool, SerializedEntityParser parser)
			throws IOException;
	...
	private void readClassAnnotation(DataInputStream din, SerializedEntityParser parser) throws IOException {
		while (true) {
			...
			case ObjectStreamConstants.TC_BLOCKDATA:
				classAnnotations.add(readBlockData(din, din.readUnsignedByte()));
				break;
	...
	/** {@inheritDoc} */
	@Override
	protected final void serializeContent(DataOutputStream dos, SerializedEntitySerializer serializer)
			throws IOException {
	...
	protected abstract void serializeClass(DataOutputStream dos, SerializedEntitySerializer serializer)
			throws IOException;
	...
	private void serializeSuperClass(DataOutputStream dos, SerializedEntitySerializer serializer) throws IOException {
		SerializedEntitySerializer.serializeObject(superClassHandle, SerializedClassBase.class, pool, dos, serializer);
	}

	/** Serializes the class annotation part. */
	private void serializeClassAnnotation(DataOutputStream dos, SerializedEntitySerializer serializer)
			throws IOException {
