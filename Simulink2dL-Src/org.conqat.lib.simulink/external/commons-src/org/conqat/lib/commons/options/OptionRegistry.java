...
public class OptionRegistry {
	...
	/**
	 * Adds all options provided by the given object to this registry. Options are
	 * public methods annotated with AOption.
	 * 
	 * @param optionHandler
		...
		for (Method m : optionHandler.getClass().getMethods()) {
			...
			if (optionDescriptor != null) {
				...
				if (optionDescriptor.shortName() != 0) {
					registerApplicator(applicator, shortOptions, optionDescriptor.shortName());
				}
				if (optionDescriptor.longName().length() > 0) {
					registerApplicator(applicator, longOptions, optionDescriptor.longName());
				}
			}
		}
	}
	...
	/**
	 * Registers an applicator under the given name in a map. Throws an exception if
	 ...
	 */
	private static <T> void registerApplicator(OptionApplicator applicator, Map<T, OptionApplicator> map,
			T optionName) {
	 ...
	 * @return the OptionApplicator for the given short option name or null if no
	 *         such options exists.
	 */
