...
public class CommandLine {
	...
	/**
	 * Parses the given command line parameters and applies the options found. The
	 * arguments not treated as options or parameters are returned (often they are
	 * treated as file arguments). If the syntax does not conform to the options in
	 ...
	 * applies the options found. The arguments not treated as options or parameters
	 * are returned (often they are treated as file arguments). If the syntax does
	 * not conform to the options in the registry an IllegalArgumentException is
	 * thrown.
	 * 
	 * @param ts
	 ...
	 */
	private static void applyOption(OptionApplicator applicator, String optionName, CommandLineTokenStream ts)
			throws OptionException {
		if (applicator == null) {
			...
			do {
				...
				try {
					...
					throw new OptionException(
							"Parameter " + parameter + " for option " + optionName + " is not of required type!");
				}
	...
	public void printUsage(PrintWriter pw, int firstCol, int width) {
		List<AOption> sortedOptions = new ArrayList<AOption>(registry.getAllOptions());
	 ...
	 *            the maximal width of a line (aka terminal width).
	 */
	private static void printOption(AOption option, PrintWriter pw, int firstCol, int width) {
	 ...
	 */
	private static String formatNames(AOption option) {
		String names = "  ";
		if (option.shortName() == 0) {
			names += StringUtils.fillString(2 + formatShortOption('x').length(), ' ');
		} else {
