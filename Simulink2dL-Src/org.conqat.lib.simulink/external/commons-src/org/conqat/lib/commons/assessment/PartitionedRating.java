...
public class PartitionedRating {
	...
	private static final Pattern PARTITIONED_RATING_PATTERN = Pattern
			.compile("(.*)" + PARTITIONED_RATING_TAG + "[ _]+(\\S+) *", Pattern.CASE_INSENSITIVE);
	...
	/**
	 * Contains the lines that make up the content of the entire file (and contain
	 * no tags)
	 */
	 ...
	 * content holding one of the rating or partition tags. The list is created and
	 * managed in a way to keep it sorted.
	 */
	 ...
	 */
	private final CollectionMap<String, RatingPartition, Queue<RatingPartition>> storedPartitions = CollectionMap
			.createWithCollectionSupplier(() -> new LinkedList<RatingPartition>());
	...
	/** Constructor. */
	public PartitionedRating(String content) throws PartitioningException, ExternalRatingTableException {
	 ...
	 */
	public PartitionedRating(String content, Class<? extends IRatingPartitioner> partitionerClass)
			throws PartitioningException, ExternalRatingTableException {
		this(StringUtils.splitLinesAsList(content), partitionerClass, null);
	}

	/** Constructor. */
	public PartitionedRating(List<String> lines) throws PartitioningException, ExternalRatingTableException {
	 ...
	 */
	public PartitionedRating(List<String> lines, Class<? extends IRatingPartitioner> partitionerClass,
			IRatingTableFileAccessor accessor) throws PartitioningException, ExternalRatingTableException {
	...
	/**
	 * Preprocesses the content's lines by filtering out all tags and preparing the
	 * {@link #contentLines}, {@link #tagLineNumbers} and {@link #storedPartitions}.
	 */
	private void preprocessLines(List<String> lines) throws PartitioningException, ExternalRatingTableException {
		int lineNumber = 0;
	...
	/**
	 * Processes the rating tag. Returns true if the tag was found and processed.
	 */
	...
	private boolean processRatingTag(String line) throws PartitioningException {
		...
		try {
			partitionerClass = (Class<? extends IRatingPartitioner>) Class.forName(className, true,
					Thread.currentThread().getContextClassLoader());
		} catch (ClassNotFoundException e) {
			throw new PartitioningException("Could not find partitioner class " + className, e);
		}
		if (!IRatingPartitioner.class.isAssignableFrom(partitionerClass)) {
			throw new PartitioningException("Given partitioner class " + className + " does not implement "
	...
	/**
	 * Processes a partition tag. Returns true if the tag was found and processed.
	 */
	...
	private void createPartitions() throws PartitioningException {
		for (Region region : splitIntoRegions()) {
			Queue<RatingPartition> queue = storedPartitions.getCollection(region.getOrigin());
			RatingPartition partition;
			if (queue != null && !queue.isEmpty()) {
				partition = queue.poll();
			} else {
				partition = new RatingPartition(null, StringUtils.EMPTY_STRING, region.getOrigin());
			}

			partition.setLinesAndContent(getOriginalLine(region.getStart()), getOriginalLine(region.getEnd()),
					contentLines.subList(region.getStart(), region.getEnd() + 1));
			partitions.add(partition);
		}
	}

	/**
	 * Splits the {@link #contentLines} into regions using the {@link #partitioner}.
	 */
	private List<Region> splitIntoRegions() throws PartitioningException {
		List<Region> regions = partitioner.partition(CollectionUtils.toArray(contentLines, String.class));
	...
	/**
	 * Returns the list of partitions. While the list itself may not be changed, the
	 * rating color of the partitions may be changed.
	 */
	...
	/**
	 * Returns the content but with the regions updated to reflect the new rating
	 * colors. If there was no old rating found for replacement, null is returned to
	 * indicate this problem. This also updates the external table if used.
	 */
	public String getUpdatedContent() {
		...
		return StringUtils.concat(output, StringUtils.LINE_SEPARATOR);
	}

	/**
	 * Inserts the partition rating table and updates the external table if used.
	 */
