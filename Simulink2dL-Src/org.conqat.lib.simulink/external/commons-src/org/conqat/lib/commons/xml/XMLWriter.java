...
public class XMLWriter<E extends Enum<E>, A extends Enum<A>> implements Closeable {
	 ...
	 * attributes.
	 * <p>
	 * We can not use {@link java.util.EnumSet} here as we would need a reference to
	 * the defining class.
	 */
	...
	/**
	 * This stack maintains the elements in order of creation. It is used to check
	 * if elements are closed in the correct order.
	 */
	...
	public XMLWriter(OutputStream stream, IXMLResolver<E, A> xmlResolver) {
		try {
			this.writer = new PrintWriter(new OutputStreamWriter(stream, FileSystemUtils.UTF8_ENCODING));
	...
	/**
	 * Toogle line break behavior. If set to <code>true</code> the writer does not
	 * write line breaks. If set to <code>false</code> (default) line breaks are
	 * written.
	 * <p>
	 * This can, for example, be used for HTML where line breaks sometimes change
	 * the layout.
	 */
	...
	public void addHeader(String version, String encoding) {
		if (state != EState.DOCUMENT_START) {
			throw new XMLWriterException("Can be called at the beginning of a document only.",
	 ...
	 *             {@link EXMLWriterExceptionType#ATTRIBUTE_OUTSIDE_ELEMENT}) or if
	 ...
	 *            where the first, third, etc. argument is an attribute enum.
	 */
	public void openElement(E element, Object... attributes) {
		if (attributes.length % 2 != 0) {
			throw new XMLWriterException("Expected an even number of arguments!",
					EXMLWriterExceptionType.ODD_NUMBER_OF_ARGUMENTS);
		}
		for (int i = 0; i < attributes.length; i += 2) {
			if (!xmlResolver.getAttributeClass().isAssignableFrom(attributes[i].getClass())) {
				throw new XMLWriterException(
						"Attribute name (index " + i + ") must be of type " + xmlResolver.getAttributeClass().getName(),
	...
	public void closeElement(E element) {
		if (element != elementStack.peek()) {
			throw new XMLWriterException("Must close element " + elementStack.peek() + " first.",
	...
	public void addComment(String text) {
		...
		printIndent();
		print(COMMENT_START);
		// XML comments must not include --
		print(escape(text.replace("--", "__")));
		print(COMMENT_END);
		currentNestingDepth--;
	...
	public void close() {
		if (!elementStack.isEmpty()) {
			throw new XMLWriterException("Need to close element <" + xmlResolver.resolveElementName(elementStack.peek())
					+ "> before closing writer.", EXMLWriterExceptionType.UNCLOSED_ELEMENT);
		}
		writer.close();
	}
	...
	public static String escape(String text) {
		...
		text = StringUtils.replaceLineBreaks(text, LINE_SEPARATOR);
		return text;
	}
	...
	private void printIndent() {
		if (!suppressLineBreaks) {
			writer.print(StringUtils.fillString(currentNestingDepth * 2, StringUtils.SPACE_CHAR));
		}
	}
